...
*v3-corpus* papers whose `referenced_works` includes BOTH the creativity
...
def main():
    ...
    for (cid, did), count in pair_counts.items():
        ...
        rows.append({
            ...
            "dd_in_v3_corpus": dm.get("in_v3_corpus"),
            "within_v3_corpus_co_citers": count,
        })
    df_pairs = pd.DataFrame(rows).sort_values("within_v3_corpus_co_citers", ascending=False)
    df_pairs.to_csv(PROC / "co_citation_v3_within_corpus_pairs.csv", index=False)

    # Aggregate by figure group
    fg = df_pairs.groupby("figure_group")["within_v3_corpus_co_citers"].sum().sort_values(ascending=False)
    fg.to_csv(PROC / "co_citation_v3_within_corpus_by_figure_group.csv")
    print("\nWithin-v3-corpus co-citations (sum across all 22 DD canonicals):")
    print(fg.to_string())

    # Per-figure-group counts
    fg_count = df_pairs.groupby("figure_group").size().sort_values(ascending=False)
    print("\nNumber of distinct (creativity, DD) pairs with within-v3-corpus co-citation > 0, by figure group:")
    print(fg_count.to_string())

    # All bridging papers (within-v3-corpus only)
    bridging_rows = []
    ...
    if bridging_rows:
        ...
        bp.to_csv(PROC / "co_citation_v3_within_corpus_bridging_papers.csv", index=False)
        print(f"\nBridging papers within v3 corpus: {len(bp)} (papers that reference ≥1 creativity canonical AND ≥1 DD/methodology canonical)")
        ...
        print("\nNo within-v3-corpus bridging papers found.")
