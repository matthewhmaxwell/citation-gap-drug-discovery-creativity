...
changes is the primary-corpus overlap intersection (since v3 corpus
...
def main():
    ...
    v3_corpus = pd.read_parquet(RAW / "primary_corpus_v3.parquet")
    v2_ids = set(v3_corpus["id"].tolist())

    # We need to know which citers are in v3 corpus.
    ...
    #   for each canonical id, the v3-corpus papers citing it ARE the citers in v2 ∩ that work's citers.
    direct_v2 = pd.read_csv(PROC / "direct_citation_v3_per_canonical.csv")
    direct_v2_lookup = direct_v2.set_index("canonical_id")["v3_corpus_citers"].to_dict()
    ...
    for _, r in v1_forward.iterrows():
        ...
        rows.append({
            ...
            "v3_corpus_citers": v2_primary,
            "v1_share": v1_primary / v1_total if v1_total else 0.0,
            "v3_share": v2_primary / v1_total if v1_total else 0.0,
            "delta_share": (v2_primary - v1_primary) / v1_total if v1_total else 0.0,
        })

    fdf = pd.DataFrame(rows)
    fdf.to_csv(PROC / "forward_citation_v3_with_v1_compare.csv", index=False)

    print(f"Targets: {len(fdf)}")
    print(f"Mean v1 primary-share: {fdf['v1_share'].mean()*100:.4f}%")
    print(f"Mean v2 primary-share: {fdf['v3_share'].mean()*100:.4f}%")
    print(f"Mean delta share: {fdf['delta_share'].mean()*100:.4f}%")

    print("\nTop 10 by v3-corpus citers:")
    print(fdf.sort_values("v3_corpus_citers", ascending=False).head(10)[
        ["figure_group","individual_name","canonical_title","canonical_year",
         "openalex_total_citers","v1_corpus_citers","v3_corpus_citers"]
    ].to_string(index=False))

    print("\nWorks where v2 share > v1 share (i.e., v2 expansion adds citers):")
    delta_pos = fdf[fdf["v3_corpus_citers"] > fdf["v1_corpus_citers"]]
    ...
    print(delta_pos.sort_values("delta_share", ascending=False)[
        ["figure_group","individual_name","canonical_title","openalex_total_citers",
         "v1_corpus_citers","v3_corpus_citers"]
    ].to_string(index=False))
