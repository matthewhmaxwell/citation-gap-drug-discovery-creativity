...
def main():
    primary = pd.read_parquet(RAW / "primary_corpus_v3.parquet")
    ...
    primary_sub["community"] = "drug-discovery-AI-v3"
    ...
    print(f"  drug-discovery-AI-v3: {(union['community']=='drug-discovery-AI-v3').sum()}")
    ...
    info.to_csv(PROC / "topic_info_v3.csv", index=False)
    union[["id","title","publication_year","venue","community","topic"]].to_csv(
        PROC / "paper_topic_assignments_v3.csv", index=False
    )
    ...
    for tid in ct.index:
        if tid == -1:
            continue
        row = ct.loc[tid]
        dd = int(row.get("drug-discovery-AI-v3", 0))
    ...
    shared_df.to_csv(PROC / "shared_topics_v3.csv", index=False)
