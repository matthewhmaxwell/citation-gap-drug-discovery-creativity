"""
3.2 v2 Co-citation analysis using the 22-paper canonical set.

Same chained-cites approach as v1. The right side now includes 10
chemistry-AI methodology canonicals.
"""
...
# Tight HTTP timeout to avoid hangs
...
def main():
    ...
    print(f"Creativity canonicals: {len(creat_unique)}")
    print(f"DD/methodology canonicals: {len(dd)}")
    n_pairs = len(creat_unique) * len(dd)
    print(f"Pairs: {n_pairs}")

    # Incremental save support: resume from existing partial CSV if present
    partial_path = PROC / "co_citation_v2_pairs.csv"
    done_keys = set()
    rows = []
        ...
        if c["global_cited_by_count"] < 5:
            for d in dd:
                ...
                rows.append({
                    ...
                    "dd_in_v1_corpus": d.get("in_v1_corpus", False),
                    "dd_in_v2_corpus": d.get("in_v2_corpus", False),
                    "dd_title": d["title"],
                    "dd_year": d["year"],
                    "co_citation_count": 0,
    ...
    df.to_csv(PROC / "co_citation_v2_pairs.csv", index=False)
    ...
    fg.to_csv(PROC / "co_citation_v2_by_figure_group.csv", index=False)

    # Split by v1-named vs v2-method canonical
    v2_only_method = df[df["dd_label"].str.contains("Coley|Schwaller|ChemCrow|Coscientist|Burger|Molecular Transformer|AlphaFold|RFdiffusion|MoleculeNet|top-cited-v2-only", regex=True, na=False)]
    print(f"\nCo-citation totals on v2 method canonicals only:")
    fg_method = (
        v2_only_method[~v2_only_method["skipped_low_cites"]]
        .groupby(["figure_group"], as_index=False)
        .agg(
            total_co_cites=("co_citation_count", lambda s: int(s[s >= 0].sum())),
            n_dd_papers_with_any_co_cite=("co_citation_count", lambda s: int((s > 0).sum())),
    ...
    print(fg_method.head(15).to_string(index=False))

    nonzero_pairs = df[df["co_citation_count"] > 0]
    print(f"\nPairs with co-cite > 0: {len(nonzero_pairs)} of {(~df['skipped_low_cites']).sum()} pairs tested.")

    # Fetch bridging papers for nonzero pairs
    bridging = []
    ...
    bp_df.to_csv(PROC / "co_citation_v2_bridging_papers.csv", index=False)
    print(f"\nBridging papers: {len(bp_df)}")

    print("\nFigure groups by total co-citations (v2):")
