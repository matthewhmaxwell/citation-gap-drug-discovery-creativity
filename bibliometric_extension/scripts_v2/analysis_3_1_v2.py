...
def main():
    ...
    rows_canon = [{
        ...
        "v2_corpus_citers": counts.get(cid, 0),
    } for cid, meta in canon_id_to_meta.items()]
    canon_df = pd.DataFrame(rows_canon).sort_values(
        ["figure_row_id", "v2_corpus_citers", "global_cited_by_count"],
        ascending=[True, False, False],
    )
    canon_df.to_csv(PROC / "direct_citation_v2_per_canonical.csv", index=False)
    ...
    rows_fig = [{
        ...
        "n_works_with_any_v2_citation": sum(1 for w in rec["canonical_works"] if counts.get(w["id"], 0) > 0),
        "total_v2_corpus_figure_paper_pairs": sum(counts.get(w["id"], 0) for w in rec["canonical_works"]),
        "unique_v2_corpus_papers_citing_figure": len(figure_pair_counts_corpus.get(fid, set())),
    } for fid, rec in canon.items()]
    fig_df = pd.DataFrame(rows_fig).sort_values("unique_v2_corpus_papers_citing_figure", ascending=False)
    fig_df.to_csv(PROC / "direct_citation_v2_per_figure_row.csv", index=False)
    ...
    fg_agg = fig_df.groupby("figure_group").agg(
        ...
        n_works_with_any_v2_citation=("n_works_with_any_v2_citation", "sum"),
        total_v2_corpus_figure_paper_pairs=("total_v2_corpus_figure_paper_pairs", "sum"),
        unique_v2_corpus_papers_citing_figure=("unique_v2_corpus_papers_citing_figure", "max"),
    ).reset_index().sort_values("total_v2_corpus_figure_paper_pairs", ascending=False)
    fg_agg.to_csv(PROC / "direct_citation_v2_per_figure_group.csv", index=False)
    ...
    for cid, lst in citing_lists.items():
        ...
        citing_dump[cid] = {
            ...
            "n_citing_v2_corpus_papers": len(lst),
            "citing_papers": lst,
        }
    (PROC / "direct_citation_v2_citing_papers.json").write_text(
    ...
    v1_v2_canon = pd.merge(
        ...
        canon_df[["canonical_id","v2_corpus_citers"]],
        on="canonical_id",
        how="outer",
    )
    v1_v2_canon["delta_v2_minus_v1"] = v1_v2_canon["v2_corpus_citers"].fillna(0) - v1_v2_canon["v1_corpus_citers"].fillna(0)
    v1_v2_canon = v1_v2_canon.sort_values("delta_v2_minus_v1", ascending=False)
    v1_v2_canon.to_csv(PROC / "v1_vs_v2_per_canonical.csv", index=False)
    ...
    v1_v2_fg = pd.merge(
        ...
        fg_agg.rename(columns={
            "total_v2_corpus_figure_paper_pairs":"v2_total_pairs",
            "unique_v2_corpus_papers_citing_figure":"v2_unique_papers",
            "n_works_with_any_v2_citation":"v2_n_works_cited",
    ...
    v1_v2_fg.to_csv(PROC / "v1_vs_v2_per_figure_group.csv", index=False)

    # Headline
    n_corpus = len(df)
    n_canonicals_cited = int((canon_df["v2_corpus_citers"] > 0).sum())
    n_corpus_papers_citing = len(set(p["corpus_paper_id"] for lst in citing_lists.values() for p in lst))
    print(f"\n=== v2 Headline ===")
    print(f"v2 corpus: {n_corpus:,} papers")
    print(f"Canonical works cited by ≥1 v2 corpus paper: {n_canonicals_cited}")
    print(f"Unique v2 corpus papers citing ≥1 canonical: {n_corpus_papers_citing} ({100*n_corpus_papers_citing/n_corpus:.3f}%)")

    print(f"\nTop 10 figure groups by total v2-corpus pairs:")
    print(fg_agg.head(10)[["figure_group","total_v2_corpus_figure_paper_pairs","unique_v2_corpus_papers_citing_figure"]].to_string(index=False))
