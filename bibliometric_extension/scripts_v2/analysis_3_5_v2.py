...
Reuses v1 fuzzy patterns; runs them against the v2 corpus.
...
def main():
    ...
    citing = json.loads((PROC / "direct_citation_v2_citing_papers.json").read_text())
    ...
    out_df.to_csv(PROC / "counterexample_v2_classified.csv", index=False)
    ...
    # Fuzzy scan
    print("\nFuzzy concept scan over v2 corpus (~33K papers)...")
    ...
    pd.DataFrame(summary_rows).to_csv(PROC / "fuzzy_scan_v2_with_v1_compare.csv", index=False)
    (PROC / "fuzzy_concept_hits_v2.json").write_text(json.dumps(concept_hits, indent=2, ensure_ascii=False))
    (PROC / "fuzzy_surname_hits_v2.json").write_text(json.dumps(surname_hits, indent=2, ensure_ascii=False))
